...
import { CAUSE_TEXT, signMessage, verifyMessage } from './mac'
...
describe('KMAC sign / verify', () => {
  ...
  it('the tag it produces is the real KMAC of the inputs', () => {
    const params = base()
    expect(signMessage(params).tagHex).toBe(toHex(kmac(params)))
  })
